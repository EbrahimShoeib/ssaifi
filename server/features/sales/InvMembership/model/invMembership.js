...
function createNewInvMembership(obj){
    const schema = joi.object({
        ...
        status:joi.string().required().valid("active","expired").min(1).max(40)
...
function updateInvMembership(obj){
    const schema = joi.object({
        clientId:joi.string().min(1).max(40),
        membershipType:joi.string().min(1).max(40),
        startDate:joi.string().min(1).max(40),
        endDate:joi.string().min(1).max(40),
        status:joi.string().valid("active","expired").min(1).max(40)
